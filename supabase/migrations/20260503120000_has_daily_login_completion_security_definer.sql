-- Align has_daily_login_completion_for_current_utc_day with submit_mission_completion visibility:
-- INVOKER + missions JOIN respected missions RLS ("available" missions only), producing
-- hasCompletion=false while SECURITY DEFINER submit still counted completions → MISSION_USER_LIMIT_REACHED.
