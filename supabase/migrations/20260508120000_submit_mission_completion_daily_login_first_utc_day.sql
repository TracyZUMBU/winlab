...
begin
  ...
  if v_mission.mission_type = 'daily_login' then
    -- No reward on the UTC calendar day of profile creation (first eligible day is the next UTC day).
    select p.created_at
    ...
    if v_profile_created_at is not null
       and (now() at time zone 'UTC')::date <= (v_profile_created_at at time zone 'UTC')::date then
      return query select false, null::uuid, 'DAILY_LOGIN_INELIGIBLE_FIRST_UTC_DAY'::text;
      return;
    end if;
  ...
  -- Lifetime/global quotas do not apply to daily_login; repeats are gated only by the UTC-day block above.
  if v_mission.mission_type <> 'daily_login'::public.mission_type then
    ...
    -- check if total mission completion limit reached across all users
    if v_mission.max_completions_total is not null then
      select count(*)
      into v_total_count
      from public.mission_completions
