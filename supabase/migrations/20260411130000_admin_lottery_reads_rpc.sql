-- Admin read RPCs for Winlab backoffice (SECURITY DEFINER + is_admin guard).
-- Views admin_* remain for transition but are legacy; the admin app should use these RPCs.
...
COMMENT ON VIEW public.admin_lotteries_overview IS
  'Legacy / transition: équivalent à public.admin_get_lotteries(). Préférer la RPC en prod ; la vue pourra être retirée.';

COMMENT ON VIEW public.admin_lottery_detail IS
  'Legacy / transition: équivalent à public.admin_get_lottery_detail(uuid). Préférer la RPC ; la vue pourra être retirée.';
