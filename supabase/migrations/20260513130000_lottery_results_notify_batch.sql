-- Phase C (SQL) : création atomique d’un run + items « pending », finalisation ;
-- exclusion des loteries déjà dans un run non terminé (évite double prise en charge par le cron Edge).
...
-- ---------------------------------------------------------------------------
-- Crée un run + une ligne pending par loterie (même critère d’éligibilité, transaction unique).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Marque les items pending du run en completed et pose finished_at sur le run.
-- Retourne les ROW_COUNT des deux UPDATE (no-op observable par le cron Edge).
-- ---------------------------------------------------------------------------
