-- Postgres cannot change a function return type with CREATE OR REPLACE.
-- We must drop the previous signature (RETURNS uuid) then create the new one.
DROP FUNCTION IF EXISTS public.buy_ticket(uuid);

CREATE FUNCTION public.buy_ticket(
  p_lottery_id uuid
)
RETURNS TABLE (
  success boolean,
