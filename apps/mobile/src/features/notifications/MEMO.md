...
| **Edge cron résultats loterie**                   | `supabase/functions/lottery-results-notify-cron/index.ts` — secret **`X-Winlab-Cron-Secret`** ; planifié côté DB par **`pg_cron`** → `public.lottery_results_notify_invoke_edge_cron()` (Vault : `lottery_results_notify_cron_secret` + URL + service role). Détail : `apps/admin/src/features/lotteries/MEMO.md`. |
| **Config functions**                              | `supabase/config.toml` — `[functions.send-push-notification]`, `[functions.admin-send-push-mvp]`, `[functions.lottery-results-notify-cron]` : `verify_jwt = false` où l’auth est hors JWT (Bearer service role ou secret cron).                                                                                                                                                                                                                                                                                                                                      |
...
- Push **résultats loterie** (cron Edge + checklist testeurs internes post-release) : `apps/admin/src/features/lotteries/MEMO.md` → section **Tests internes post-release**.
